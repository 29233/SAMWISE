...
def main(args):    
    ...
    inference(args, model, save_path_prefix, input_path, args.text_prompts, args.audio_path)
...
def compute_masks(model, text_prompt, frames_folder, frames_list, audio_path, ext):
    all_pred_masks = []
    ...
    for imgs, clip_frames_ids in tqdm(dl):
        ...
        with torch.no_grad():
            outputs = model([imgs], [text_prompt], [audio_path], [target])
...
def inference(args, model, save_path_prefix, in_path, text_prompts, audio_path):
    # load data
    ...
    for i in range(len(text_prompts)):
        ...
        all_pred_masks = compute_masks(model, text_prompt, frames_folder, frames_list, audio_path, ext)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_path', default='/18018998051/Ref-AVS/data/REFAVS/media/--iSerV5DbY_68000_78000/frames', type=str, required=False, help='path to mp4 video or frames folder')
    parser.add_argument('--text_prompts', default=['The hair-dryer in front of the man.'], type=str, required=False, nargs='+', help="List of referring expressions, separated by whitespace")
    parser.add_argument('--audio_path', default='/18018998051/Ref-AVS/data/REFAVS/media/--iSerV5DbY_68000_78000/audio.wav', type=str, required=False, help='path to the audio wav file')
