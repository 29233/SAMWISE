...
from models.maskformer.msdeformattn import MSDeformAttnDINO as MSDeformAttn
from ops.modules.ms_deform_attn import MSDeformAttnDINO
...
class DINOv3_Adapter(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Conv2d(embed_dim, d_model, kernel_size=1)
        self.fc2 = nn.Conv2d(embed_dim, d_model, kernel_size=1)
        self.fc3 = nn.Conv2d(embed_dim, d_model, kernel_size=1)
        self.fc4 = nn.Conv2d(embed_dim, d_model, kernel_size=1)

        self.norm1 = nn.SyncBatchNorm(d_model)
        self.norm2 = nn.SyncBatchNorm(d_model)
        self.norm3 = nn.SyncBatchNorm(d_model)
        self.norm4 = nn.SyncBatchNorm(d_model)
    ...
    def forward(self, x):
        ...
        f3 = self.norm3(c3)
        f4 = self.norm4(c4)
        return {"vision_features" : f4, "backbone_fpn" : [f1, f2, f3, f4], "vision_pos_enc" : [1, 2, 3, 4]}


if __name__ == "__main__":
    from dinov3.hub.backbones import dinov3_vitl16
    test_sample = torch.randn(1, 3, 256, 256)
    backbone = dinov3_vitl16()
    model = DINOv3_Adapter(backbone)
    model.eval()
    model.to("cuda")
    test_out = model(test_sample.cuda())
    print(test_out)
