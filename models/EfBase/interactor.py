...
class DualInteractor(nn.Module):
    def __init__(self,
                 num_heads: int = 8,
                 d_model: int = 256,
                 num_layers: int = 6,
    ...
    def forward(self, inter_omni_rep, omni_rep):
        ...
        inter_omni_rep = inter_omni_rep.view(B, -1, C)
        omni_rep = omni_rep.view(B, -1, C)
        # inter_omni_rep = self.inter_layers(inter_omni_rep)
        # omni_rep = self.layers(omni_rep)
        # final_rep = torch.cat([inter_omni_rep, omni_rep], dim=1)
        # final_rep = final_rep.view(B * T, -1, C)
        inter_omni_rep = self.inter_layers(inter_omni_rep).view(B * T, N, C)
        omni_rep = self.layers(omni_rep).view(B * T, N, C)
        final_rep = torch.cat([inter_omni_rep, omni_rep], dim=1)
        return final_rep


class AddInteractor(nn.Module):
    def __init__(self,
                 num_heads: int = 8,
                 d_model: int = 256,
                 num_layers: int = 6,
    ...
    def forward(self, inter_omni_rep, omni_rep):
        B, T, N, C = inter_omni_rep.shape
        omni_rep = (omni_rep + inter_omni_rep).view(B, -1, C)
